...
- **全文翻译：** 除专有名词外，正文全部译为自然流畅的中文
- **保留英文的专有名词：** 产品名（GMV Max、Brand Max 等）、指标名（ROAS、CTR、CPM、GMV 等）、品牌名（Shopee、Facebook、Google 等）、功能特性名（Enhanced Bid Price、Smart Voucher 等）
- **图片处理：** 原始内嵌 Base64 图片已替换为 `[Image]` 占位符，外部图片链接保留原 URL
- **格式：** 内容从 HTML 转为 Markdown，保留原文链接供对照
